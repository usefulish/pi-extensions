...
/**
 * Contract: a normal return asserts the turn finished (or needs input). If
 * the abort signal fired first, the runner MUST throw — messageSend routes
 * any normal return after an abort through the failure classification
 * anyway, and an honest throw carries the real error (timeout vs cancel).
 */
